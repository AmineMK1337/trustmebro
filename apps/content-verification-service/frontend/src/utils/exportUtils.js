...
export const exportResults = (result, fileName) => {
  ...
  const exportFileDefaultName = `trust-me-bro_analysis_${fileName}_${new Date().toISOString().slice(0,10)}.json`;
...
export const downloadPDF = (result, fileName, preview) => {
  ...
  pdf.text('Trust Me Bro Analysis Report', 20, 20);
  ...
  pdf.text('Generated by Trust Me Bro - Advanced AI Deepfake Detection', 20, 280);
  
  // Save the PDF
  pdf.save(`trust-me-bro_report_${fileName}_${new Date().toISOString().slice(0,10)}.pdf`);
};
