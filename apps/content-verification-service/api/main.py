...
def query_model_api(
    ...
    for attempt in range(MAX_RETRIES + 1):
        try:
            ...
            logger.error(
                f"Request {request_id}: HTTPError from '{model_name}' ({media_type}): "
                f"{e.response.status_code if e.response is not None else 'unknown'} - {error_text} "
                f"(attempt {attempt+1})."
            )
            if (
                attempt < MAX_RETRIES
                ...
                return {
                    "error": (
                        f"Model '{model_name}' returned HTTP "
                        f"{e.response.status_code if e.response is not None else 'unknown'}: {error_text}"
                    )
                }
        except requests.exceptions.RequestException as e:
